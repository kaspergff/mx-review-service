# System Prompt als bestand

## Doel

De LLM system prompt verplaatsen van een hardcoded string in `server.py` naar een extern `.md` bestand zodat de prompt makkelijk te testen en itereren is zonder code-wijzigingen. De inhoud is gebaseerd op de officiële Mendix best practices documentatie.

## Bestandsstructuur

```
prompts/
  system_prompt.md   # de volledige LLM system prompt
```

## Code-wijziging in server.py

- Verwijder de `SYSTEM_PROMPT` constante (regels 191-198)
- Voeg `_load_system_prompt()` toe: leest `prompts/system_prompt.md` via `pathlib.Path`
- Valideer bij startup dat het bestand bestaat (gooit `RuntimeError` als het ontbreekt)
- `review_diff()` roept `_load_system_prompt()` aan bij elke request (zodat wijzigingen direct effect hebben)

## Inhoud system_prompt.md
...
- Ken de retrieve + count optimalisatie: als je een lijst ophaalt én telt, maakt Mendix er één query van
...
Het bestand wordt bij **elke request** ingelezen zodat wijzigingen direct effect hebben zonder herstart.
